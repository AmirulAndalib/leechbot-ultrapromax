...
@Client.on_message(filters.command(['directdl', 'direct', 'zipdirectdl', 'zipdirect']) & filters.chat(ALL_CHATS))
...
- /zipdirect <i>(as reply to a Direct URL) | optional custom file name</i>''')
    ...
    if parsed[0] == 'magnet':
        await message.reply_text(f'Use /{"zip" if send_as_zip else ""}magnet instead')
        return
    if not parsed[0]:
        parsed[0] = 'https'
    ...
    await initiate_directdl(client, message, link, filename, send_as_zip)

async def initiate_directdl(client, message, link, filename, send_as_zip):
    user_id = message.from_user.id
